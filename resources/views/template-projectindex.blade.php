    ...
    <div class="px-4 container mb-36 max-w-5xl space-y-24 lg:mt-48">
      ...
      @if($loop->index % 2)
          <a href="{{ get_permalink($project->ID) }}"
            ...
            <div class="flex flex-col items-start justify-center md:w-2/3 @if($loop->index % 3) bg-axis-green @else bg-axis-purple @endif p-12 md:pr-36">
              <h3 class="mb-6 font-mono text-4xl ">{{ $project->post_title }}</h3>
              <p>
                {{ $project->post_excerpt }}
              </p>
              <span class="mr-auto transform transition duration-500 ease-out group-hover:translate-y-2 mt-4 -ml-16 inline-block antialiased font-semibold rounded-r-full bg-white px-16 py-4">Read more</span>
            </div>
        ...
        @else
            ...
            <div class="w-full md:w-1/2 px-8 md:mx-0">
              ...
              <span class="relative z-10 transform transition duration-500 ease-out group-hover:translate-y-2 mt-4 -ml-24 inline-block antialiased font-semibold rounded-r-full bg-axis-yellow px-16 py-4">Read more</span>
            </div>
</a>
        @endif
      @endforeach
