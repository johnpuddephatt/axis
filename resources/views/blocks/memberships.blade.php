...
<div class="container relative py-24 grid lg:grid-cols-3 gap-8 lg:gap-y-16">
@foreach($fields['memberships'] as $membership)
@php $post = get_post($membership['id']) @endphp
<div class="flex flex-row rounded-3xl bg-white @if($loop->iteration == 1) lg:col-span-3 @endif">
    <div class="px-8 py-12 flex-1">
<h3 class="mt-0 text-4xl mb-0">{{ get_the_title($post->ID) }}</h3>
<p class="font-bold text-axis-purple text-lg">£{{ carbon_get_post_meta($post->ID, 'cost') }} per month</p>
<div class="font-bold max-w-sm">{{ carbon_get_post_meta($post->ID, 'description') }}</div>
<div><button class="antialiased font-semibold transform transition duration-500 ease-out  group-hover:translate-y-2 mt-6 -ml-16 inline-block mr-3 rounded-r-full bg-axis-yellow px-16 py-4"
 ...
 @if($loop->iteration == 1)
 <div class="overflow-hidden w-1/3 flex-none bg-gray-100 rounded-r-3xl relative">
